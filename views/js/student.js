...
const form5 = document.getElementById('dm')
const dm_btn = document.getElementById('dm_btn')

function dm(event){
    event.preventDefault()
    const input = form5.querySelector('input')
    message = input.value
    socket.emit('to_teacher', myTeacher, myNick, message)
    input.value=''
}
dm_btn.addEventListener('click', dm)
...
socket.on('teacherName', (teacherName)=>{
    myTeacher = teacherName
    console.log(myTeacher)
})

socket.on('teacherCome', (teacherID)=>{
    addMessage('teacherCome')
})

socket.on('relogin', (coalaID)=>{
    if (confirm("재 로그인 하시겠습니까?")) {
        socket.emit('relogin', coalaID, false)
    }
})
